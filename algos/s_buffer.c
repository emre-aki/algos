...
static span_t* SB_Node (int start, int size, byte id)
{
    span_t* node = (span_t*) E_Malloc(sizeof(span_t), SB_Node);
...
int SB_Push (sbuffer_t* sbuffer, int start, int size, byte id)
{
    ...
    while (remaining > 0)
    {
        ...
        if (clipped_size > 0)
        {
            span_t* new_node = SB_Node(offset + clipleft, clipped_size, id);
